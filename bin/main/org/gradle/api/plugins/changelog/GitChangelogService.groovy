...
class GitChangelogService {
    ...
    static def headerTemplate = '<%= versionText %> (<%= date %>)\n\n' //'<a name="<%= version %>"></a>\\n<%= versionText %> (<%= date %>)\n\n'
    ...
    def writeChangelog(RandomAccessFile fw, List commits, Map opts) {
        ...
        def start = opts.headerSeek
        def size = fw.length().toInteger()-1
        def b = new byte[fw.length()]
        println("start? ${start}")
        println("size? ${size}")
        println("Lenght? ${fw.length()}")
        ...
        byte[] oldStuff = b[start..size].toArray()
        fw.write(oldStuff)
        fw.close()
    }
